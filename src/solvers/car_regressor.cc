...
static bool& g_use_curvature_dependent_segmentation = CVarUtils::CreateGetCVar("learning.CurvatureDependentSegmentation",false);
static int& g_segment_length = CVarUtils::CreateGetCVar("learning.SegmentLength",80);
...
struct ApplyParametersThreadFunctor {
  ...
  void operator()()
  {
    ...
    if( index_ >= m_f.vehicle()->GetWorldCount()){
      assert(false);
    }
...
{
  segment_length_ = g_segment_length;
  ...
  for(size_t startIndex = 0 ;  startIndex < sample.commands_vector_.size() ; ){
    ...
    if(g_use_curvature_dependent_segmentation){
      for(size_t jj = endIndex; jj < sample.commands_vector_.size() ; jj++){
        ...
        if(fabs(sample.commands_vector_[jj].curvature_) < 0.5 ){
          ...
          for(size_t kk = jj ;
              ...
              dDelay < f.vehicle()->GetParameters(0)[VehicleParameters::ControlDelay]  ;
              kk++){
            endIndex = jj;
          }
          break;
                                         ...
                                         std::vector<int>* motion_sampleIndices/* = NULL */)
{
  ...
  for(size_t ii = 0; ii < segment_indices_.size() ; ii++){
    ...
    if(worldCounter >= simFunctor.vehicle()->GetWorldCount()) {
                                   ...
                                   CommandList *pPreviousCommands /*= NULL*/)
{
  ...
  f.vehicle()->UpdateParameters(params,index);
                                     ...
                                     Eigen::VectorXd& Jb)
{
  ...
  for(size_t jj = 0; jj < segment_indices_.size() ; jj++){
    ...
    for(size_t ii = 0 ; ii < params.size() ; ii++) {
      //if we have no more worlds, wait until all worlds have finished
      if(worldCounter >= functor.vehicle()->GetWorldCount()) {
        while(thread_pool_.busy_threads() > (thread_pool_.num_threads())){
          std::this_thread::sleep_for(std::chrono::milliseconds(10));
        }
        worldCounter  = 0;
